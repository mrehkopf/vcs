...
def mark_details_textblock(html:str)->str:
    dom = parse_html(html)

    headingNodes = dom.select("#doc-content h2.groupheader")
    for node in headingNodes:
        if node.string and node.string.strip().lower() == "detailed description":
            node.findNext()["class"].append("vcs-section-details")
            break

    return str(dom)

# Give <code> blocks inside the "Detailed description" section a special class
# name. These are expected to have been generated by Doxygen from `` Markdown
# syntax (e.g. `hello` => <code>hello</code>).
def mark_markdown_codeblocks(html:str)->str:
    dom = parse_html(html)

    # Note: We expect mark_details_textblock() to have marked the details block
    # with this special class.
    detailsBlock = dom.select(".vcs-section-details")
    assert len(detailsBlock) <= 1, "Expected at most one details block."

    if not detailsBlock:
        return str(dom)

    markdownCodeElements = detailsBlock[0].select("code")
    
    for node in markdownCodeElements:
        assert not node.has_attr("class"), "Expected Markdown code blocks to have no class name."
        node["class"] = ["vcs-markdown-code"]
...
reduce_html("./html/", [
    ...
    mark_details_textblock,
    mark_markdown_codeblocks, # Do this before standardize_code_elements, otherwise we can't tell markdown <code> blocks from Doxygen's @code blocks.
