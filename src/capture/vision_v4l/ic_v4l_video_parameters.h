...
public:
    ...
    ic_v4l_controls_c(const int v4lDeviceFileHandle = -1);
    ...
    bool set_value(const int newValue, const type_e controlType);

    int value(const type_e control) const;

    int default_value(const type_e control) const;

    int minimum_value(const type_e control) const;

    int maximum_value(const type_e control) const;

    int v4l_id(const type_e control) const;

    std::string name(const type_e control) const;

    // Polls the capture device for the current video parameters' values.
    void update(void);

private:
    type_e type_for_name(const std::string &name);
    ...
    std::unordered_map<type_e, control_data_s> controls;
